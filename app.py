...
st.info("""
**How to use:** Adjust the inputs on the left. Switch between Hold and Flip mode to see different outputs.
""")
...
potential_equity_upside = (arv - loan_amount) - down_payment

# ===================== DISPLAY =====================
col1, col2, col3, col4 = st.columns(4)
col1.metric("Going-in Cap Rate", f"{cap_rate:.2f}%")
col2.metric("Cash-on-Cash Return", f"{cash_on_cash:.1f}%")
col3.metric("Purchase → ARV Equity Upside", f"${potential_equity_upside:,.0f}")
col4.metric("Rehab Cost per Unit", f"${rehab_cost / units:,.0f}")
...
if analysis_mode == "Hold (Rental)":
    ...
    for year in years:
        ...
        projections.append({
            'Year': year,
            'Avg Rent': round(current_rent, 2),
            'NOI': round(noi_y, 0),
            'Cash Flow': round(cf_y, 0)
        })
    ...
    est_holding_cost = annual_debt_service * 0.5  # rough 6 months
    flip_profit = arv - purchase_price - rehab_cost - est_holding_cost
    st.metric("Estimated Flip Profit (before selling costs)", f"${flip_profit:,.0f}")

st.subheader("Key Insights")
st.info(f"""
**Deal Highlights:**
- Equity upside from Purchase to ARV: **${potential_equity_upside:,.0f}**
- Rehab per unit: **${(rehab_cost/units):,.0f}**
- {'Strong Hold candidate with solid cash flow.' if analysis_mode == "Hold (Rental)" and cash_on_cash > 8 else 'Better suited as a Flip.'}
""")
